...
class RecordedCompleter:
    ...
    `usage_by_key` is what makes a replay report a cost rather than a zero. A
    recording preserves what the model SAID; without this it preserved nothing
    about what saying it cost, so every replayed ledger came back empty and the
    cross-rung table could only be regenerated by paying for it again. The
    counts live beside the recordings in `tests/recordings/usage.json`, keyed
    identically, and `scripts/build_usage.py` builds them once with
    `count_tokens` - no generation, and nothing to re-run afterwards.

    It stays OPTIONAL on purpose. Every construction site that predates it
    passes `recordings=` alone and still works, falling back to the shared
    `usage` default; a missing key is a zero rather than an exception, because a
    fake that refused to run without a cost file would make the cheap path the
    broken one.
    """

    recordings: dict[str, str]
    usage: Usage = Usage()
    usage_by_key: dict[str, Usage] = field(default_factory=dict)

    def _billed(self, *keys: str) -> Usage:
        """The recorded cost of the requests under `keys`, summed.

        Summing is not a convenience. One `invoke` is one measurement covering
        TWO vendor requests - the round that chose the tools and the round that
        read their results - which is the trap Chapter 8 named and Chapter 9
        inherited. Billing them separately here would double the ledger's call
        count against a rung that made one call through this seam.
        """
        found = [self.usage_by_key[key] for key in keys if key in self.usage_by_key]
        if not found:
            return self.usage
        return Usage(
            input_tokens=sum(each.input_tokens for each in found),
            output_tokens=sum(each.output_tokens for each in found),
        )
    ...
    def parse(
        ...
        *,
        ...
        try:
            ...
            # Billed even though it is useless. Chapter 4's point about
            # client-side validation: the tokens were spent before Pydantic
            # ever saw them, so a ledger that skipped this would under-report
            # exactly the failure that costs money and returns nothing.
            return Completion(None, self._billed(key), f"schema violated: {_first(exc)}")
        return Completion(parsed, self._billed(key))
    ...
    def invoke(
        ...
        *,
        ...
        if not calls:
            return ToolRun(
                None, self._billed(first), failed="the model asked for no tools"
            )

        results = tuple(execute(call) for call in calls)
        second = answer_key(system=system, user=user, results=results)
        if second not in self.recordings:
            # Only the first request happened. Billing the pair here would
            # invent a request the recorded run never made.
            return ToolRun(
                None, self._billed(first), calls, results,
                f"no recording for answer {second}",
            )
        body = self.recordings[second]
        if body == WANTED_MORE:
            return ToolRun(
                None,
                self._billed(first, second),
                calls,
                results,
                "one round of tools was not enough",
                wanted_more=True,
        ...
        try:
            ...
            return ToolRun(
                ...
                calls,
                results,
                f"schema violated: {_first(exc)}",
            )
        return ToolRun(parsed, self._billed(first, second), calls, results)
